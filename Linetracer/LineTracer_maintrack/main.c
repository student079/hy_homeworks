...
//                똑같이 센서켜져있으면 lap + 1
                ...
                {
                    ir_update();
                    if ((sensor2 && sensor3 && sensor4 && sensor5 && sensor6
                            && sensor7 && !sensor1 && !sensor8)
                            || (!sensor2 && sensor3 && sensor4 && sensor5
                                    && sensor6 && sensor7 && !sensor1 && sensor8)
                            || (sensor2 && sensor3 && sensor4 && sensor5
                                    && sensor6 && !sensor7 && sensor1
                                    && !sensor8))
                    {
                        lap += 1;
                        turn_on_led(3 * lap);
...
//        좌회전 90도
//        좌회전 인식 더 하도록 센서 2,3번만 들어오는 조건도 추가
        if ((sensor1 && sensor2 && sensor3) || (sensor2 && sensor3))
        {
            left_count = 0;
            while (1)
            {
                if (left_count > 60)
                {
//                    앞으로 조금 가보고 sensor4,5하나라도 안켜져있으면 길이 없는것이므로
//                    flag = 1
//                    길이 있으면 flag = 0 이라 직진
                    ir_update();
                    turn_off_led();
...
//        우회전
        ...
        if ((sensor4 || sensor5) && !sensor7 && !sensor6 && sensor8)
        {
            left_count = 0;
            turn_on_led(8);
            while (1)
            {
                if (left_count > 100 && left_count <= 400)
                {
                    left_forward();
                    right_backward();
                    move(SPEED, SPEED);
                }
                else if (left_count > 400)
                {
                    left_forward();
                    right_forward();
                    move(SPEED, SPEED);
